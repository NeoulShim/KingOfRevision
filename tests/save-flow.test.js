...
function harness(){
  ...
  function node(id){if(!nodes.has(id))nodes.set(id,{id,style:{},scrollTop:0,scrollLeft:0,open:false,value:id==='export-name'?'테스트 원고':'',textContent:'',innerHTML:'',classList:{toggle(){},add(){},remove(){}},events:{},addEventListener(type,fn){this.events[type]=fn},showModal(){this.open=true},close(){this.open=false},focus(){},click(){},setAttribute(){}});return nodes.get(id)}
...
test('applying an edit restores comparison and sidebar scroll',async()=>{const h=harness();h.model.scenes=[{new:['처음','다음'],newStart:0}];h.node('diff-pane').scrollTop=850;h.node('diff-pane').scrollLeft=12;h.node('scene-list').scrollTop=70;h.api.showEditor('revised');h.node('manuscript-editor').value='새 처음\n다음';const revised={...h.model,fingerprint:'edited-scroll'};h.context.workerCall=async type=>type==='edit'?{comparison:revised,choices:{}}:{fingerprint:'edited-scroll',documents:{}};vm.runInContext("renderWorkspace=()=>{document.getElementById('diff-pane').scrollTop=0;document.getElementById('scene-list').scrollTop=0}",h.context);await h.api.applyEditor();assert.equal(h.node('diff-pane').scrollTop,850);assert.equal(h.node('diff-pane').scrollLeft,12);assert.equal(h.node('scene-list').scrollTop,70)});
