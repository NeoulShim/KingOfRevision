// Structural matching is local and monotone: neither source text nor order is discarded.
export function heading(text){
 const value=text.trim().normalize('NFKC');if(value.length>90||/[\n\r]/.test(value))return null;
 const m=value.match(/^(?:제\s*)?([0-9]+|[일이삼사오육칠팔구십백]+)\s*(부|장|권)(?=$|\s|[:：.\-–—])\s*[:：.\-–—]?\s*(.*)$/)||value.match(/^(part|chapter|book)\s+([0-9]+|[ivxlcdm]+)(?=$|\s|[:.\-–—])\s*[:.\-–—]?\s*(.*)$/i);
 if(!m)return null;const english=/^(part|chapter|book)$/i.test(m[1]);const kind=english?({part:'부',chapter:'장',book:'권'}[m[1].toLowerCase()]):m[2];return {kind,number:(english?m[2]:m[1]).toLowerCase(),subtitle:m[3].trim(),title:value};
}
