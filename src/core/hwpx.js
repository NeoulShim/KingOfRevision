...
export function readHwpx(input, filename='원고.hwpx'){
  ...
  try{files=unzipSync(bytes,{filter:f=>{
    ...
    const wanted=f.name==='mimetype'||/^Contents\/(?:content\.hpf|section\d+\.xml)$/.test(f.name)||/^META-INF\/.*\.xml$/.test(f.name);
  ...
  function push(text,kind,section){chars+=text.length;if(chars>LIMITS.text||blocks.length>=LIMITS.paragraphs)throw Error('본문은 100만 자, 4만 문단까지 비교할 수 있습니다.');blocks.push({text,kind,section})}
  function visit(nodes,context,depth=0){
    ...
    for(const node of nodes){const name=nameOf(node);if(!name)continue;const children=Array.isArray(node[name])?node[name]:[];
      if(name==='p'){readParagraph(children,context,depth+1);continue}
  ...
  function readParagraph(nodes,context,depth){let text='',nested=false;
    const flush=()=>{if(text!==''){push(text,context.kind,context.section);text=''}};
    function walk(list,d){if(d>LIMITS.depth)throw Error('문서 구조가 너무 깊어 처리할 수 없습니다.');for(const n of list){const name=nameOf(n);if(!name)continue;const children=Array.isArray(n[name])?n[name]:[];
      if(name==='t')text+=textContent(children);
      ...
      else walk(children,d+1);
    }}walk(nodes,depth);if(text!==''||!nested)push(text,context.kind,context.section);
  }
  ...
  return {name:filename,paragraphs:blocks.map(b=>b.text),blocks,characters:chars,sections:ordered.length,warnings};
}
...
export function writeHwpx(paragraphs,{title='선택한 원고'}={}){
  ...
  const paragraphsXML=(paragraphs.length?paragraphs:['']).map((p,i)=>`<hp:p id="${i}" paraPrIDRef="0" styleIDRef="0" pageBreak="0" columnBreak="0" merged="0">${i===0?`<hp:run charPrIDRef="0">${secPr}${colPr}</hp:run>`:''}<hp:run charPrIDRef="0"><hp:t>${runText(p)}</hp:t></hp:run></hp:p>`).join('');
