...
function row(scene,a,b,marks=null){const equal=a!==null&&b!==null&&scene.old[a]===scene.new[b];const cell=(side,n,p)=>{if(n===null)return '<div class="cell blank" aria-label="대응 문단 없음"><span></span><p class="text"></p></div>';return `<div class="cell ${equal?'':side==='old'?'deleted':'added'}"><span class="line-no" aria-hidden="true">${n+1}</span><p class="text">${marked(scene[side][n],!equal&&!p?[[scene[side][n],true]]:p)}</p></div>`};return `<div class="row ${equal?'equal-row':''}">${cell('old',a,marks?.[0])}${cell('new',b,marks?.[1])}</div>`}
function paragraphBody(scene,g){
  ...
  const cell=()=>`<div class="cell resolved-cell"><span class="line-no" aria-hidden="true">✓</span><p class="text">${note?`<span class="resolved-empty">${note}</span>`:marked(state.text,null)}</p></div>`;
