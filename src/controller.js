...
function changeHeader(g){const v=choices[g.id];return `<div class="change-meta"><strong>문단 ${g.number}</strong><span class="change-type type-${g.description.kind}">${g.description.label}</span><span class="sentence-count">${g.description.before}문장 → ${g.description.after}문장</span>${g.description.structure?`<span class="structure-label">${g.description.structure}</span>`:''}${g.move?`<button class="text-btn" data-move-scene="${g.move.peerScene}" data-move-id="${g.move.peerId}">이동한 짝 보기</button>`:''}<span class="choice-status ${v||''}">${v==='new'?'✓ 개고 채택됨':v==='old'?'✓ 원문 유지됨':'미선택'}</span></div><div class="choice-actions">${g.b[1]>g.b[0]?paragraphEdit(model.scenes[sceneIndex],g.b[0]):''}<button class="choice-btn ${v==='old'?'selected':''}" data-choice="${g.id}" data-value="old" aria-pressed="${v==='old'}">${v==='old'?'✓ ':''}원문 유지</button><button class="choice-btn ${v==='new'?'selected':''}" data-choice="${g.id}" data-value="new" aria-pressed="${v==='new'}">${v==='new'?'✓ ':''}개고 채택</button><button class="text-btn clear-btn" data-choice="${g.id}" data-value="clear" aria-label="문단 ${g.number} 선택 해제" ${v?'':'disabled'}>↺</button></div>`}
...
function renderDiff(reset=true){if(!model)return;const scene=model.scenes[sceneIndex],pane=$('diff-pane'),oldScroll=pane.scrollTop;const oldNumber=scene.oldIndex===null?'없음':scene.oldIndex+1,newNumber=scene.newIndex===null?'없음':scene.newIndex+1;$('current-title').textContent=(scene.partTitle?scene.partTitle+' · ':'')+'장면 '+(sceneIndex+1);$('current-sub').textContent=`원문 ${oldNumber} → 개고 ${newNumber} · 변경 ${scene.changes}곳 · 검토 ${completed(scene)}/${scene.changes}`;
  ...
  if(scene.partKey)html+=`<div class="structure-note"><div><b>${esc(scene.partTitle)}</b> · ${scene.partStatus==='added'?'새로 추가된 부·장':scene.partStatus==='deleted'?'삭제된 부·장':'제목과 본문으로 자동 대응'}<br><small>${scene.paragraphCounts?'본문 문단 '+scene.paragraphCounts.old+' → '+scene.paragraphCounts.new+'개 · ':''}구조는 자동 비교, 반영은 직접 선택</small></div><div><button data-part-choice="old">이 부·장 원문 유지</button><button data-part-choice="new">${scene.partStatus==='deleted'?'이 부·장 삭제 채택':'이 부·장 개고 채택'}</button></div></div>`;
...
function applyChoices(items,message){if(!model)return;const next={...choices};const ids=new Set(allChanges().map(h=>h.id));const peers=new Map(allChanges().filter(g=>g.move).map(g=>[g.id,g.move.peerId]));items=items.flatMap(([id,v])=>{const peer=peers.get(id);return peer?[[id,v],[peer,v]]:[[id,v]]});for(const [id,value] of items){if(!ids.has(id)||!['old','new','clear'].includes(value))throw Error('올바르지 않은 수정 선택입니다.');if(value==='clear')delete next[id];else next[id]=value;}if(JSON.stringify(next)===JSON.stringify(choices))return;history.push({...choices});if(history.length>100)history.shift();choices=next;saveChoices();renderSidebar();const s=model.scenes[sceneIndex];$('current-sub').textContent=`원문 ${s.oldIndex===null?'없음':s.oldIndex+1} → 개고 ${s.newIndex===null?'없음':s.newIndex+1} · 변경 ${s.changes}곳 · 검토 ${completed(s)}/${s.changes}`;for(const g of s.segments)if(g.type==='change'&&$(g.id)){$(g.id).classList.toggle('resolved',!!choices[g.id]);$(g.id).innerHTML='<div class="change-head">'+changeHeader(g)+'</div>'+paragraphBody(s,g);}if(message)toast(message)}
...
document.addEventListener('click',e=>{const b=e.target.closest('button,a');if(!b)return;
  ...
  if(b.hasAttribute('data-move-scene')){navigate(model.scenes.findIndex(s=>s.id===b.dataset.moveScene),b.dataset.moveId);return}
  ...
  if(b.hasAttribute('data-part-choice')){const key=model.scenes[sceneIndex].partKey;if(key)applyChoices(model.scenes.filter(s=>s.partKey===key).flatMap(s=>s.segments.filter(g=>g.type==='change').map(g=>[g.id,b.dataset.partChoice])),'이 부·장의 선택을 반영했습니다.');return}
