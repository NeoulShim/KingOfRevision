...
function renderDiff(reset=true){if(!model)return;const scene=model.scenes[sceneIndex],pane=$('diff-pane'),oldScroll=pane.scrollTop;const oldNumber=scene.oldIndex===null?'없음':scene.oldIndex+1,newNumber=scene.newIndex===null?'없음':scene.newIndex+1;$('current-title').textContent=(scene.partTitle?scene.partTitle+' · ':'')+'장면 '+(sceneIndex+1);$('current-sub').textContent=`원문 ${oldNumber} → 개고 ${newNumber} · 변경 ${scene.changes}곳 · 검토 ${completed(scene)}/${scene.changes}`;
  ...
  if(scene.partKey)html+=`<div class="structure-note"><div><b>${esc(scene.partTitle)}</b> · ${scene.partStatus==='added'?'새로 추가된 부·장':scene.partStatus==='deleted'?'삭제된 부·장':'제목과 본문으로 자동 대응'}<br><small>구조는 자동으로 비교하며, 반영할 내용은 직접 선택합니다.</small></div><div><button data-part-choice="old">이 부·장 원문 유지</button><button data-part-choice="new">${scene.partStatus==='deleted'?'이 부·장 삭제 채택':'이 부·장 개고 채택'}</button></div></div>`;
